...
보여주게 되면서 그 이유가 사라졌다. 바뀌지 않은 것도 있다: 지오코딩에
필요한 것은 여전히 \`data/raw/\`의 거래별 원본 주소이지 여기 실린 대표
주소 하나가 아니다. 그래서 원본은 \`data/raw/\`에 그대로 남고, 여기 실리는
것은 **표시용** 문자열 하나다(위 "data/raw/*.json" 참고).
...
**\`landLeasehold\`도 거래별 주소도 \`complexes.json\`에 그대로 담기지는 않는다.**
\`landLeasehold\`는 단지 단위로 합쳐서(위 표 참고), 주소는 단지 단위 문자열
하나(\`address\`)로 만들어서 들어간다 — 거래마다 다른 \`address\` **객체**가
그대로 실리지는 않는다는 뜻이다. 지오코딩이 쓰는 것은 여전히 여기 실린 대표
주소가 아니라 \`data/raw/\`의 거래별 원본이다.
