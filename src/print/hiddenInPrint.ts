...
export const PRINT_HIDDEN_SELECTORS: readonly string[] = [
  ...
  // 종이에 빈 테두리 상자가 되고, **그 안의 마커 색 범례**(`.complex-map-legend`)
  // 도 함께 남는다 — 지도가 없는 종이 위에서 "이 색은 대출 없이 살 수
  // 있다는 뜻"이라는 안내는 가리킬 마커가 없는 고아 문구다. 액자를
  // 통째로 지우면 그 안의 범례도 함께 사라지므로 선택자를 둘로 늘리지
  // 않는다(범례에는 보호 대상 클래스가 하나도 들어 있지 않다 —
  // App.test.tsx의 DOM 교차곱 검사가 그것까지 확인한다).
  ".complex-map-frame",
