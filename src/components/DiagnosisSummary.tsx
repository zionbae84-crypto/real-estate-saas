...
export interface DiagnosisSummaryProps {
  /** 권리분석 문진 결과. 이 화면은 항상 렌더되므로 실질적으로 거의 언제나 값이 있다 */
 ...
 * ## `<details>`로 접지 않는다
 *
 * 이 저장소의 다른 큰 섹션(`RightsCheck`)은 접힌 채로 시작한다. 이
 * 화면은 그렇게 두지 않는다 — 이 화면의 존재 이유가 "못 본 축을
 * 숨기지 않는 것"인데, 화면 전체를 접어 두면 그 못 본 축조차 클릭
 * 하지 않은 사람에게는 보이지 않는다. 항상 펼쳐 둔다.
 *
 * ## 어디 있는가
 *
 * `App.tsx`에서 권리분석 문진(`RightsCheck`) 바로 다음, 화면의 맨
 * 끝(면책 문구 바로 위)에 둔다. 권리분석은 실거주·투자 어느 경로에도
 * 있고, 구매 유형별 금융(투자 경로)과 호가·입지(실거주에서 평형을
 * 고른 경우)는 서로 배타적이라 실제로 넷이 동시에 채워지는 일은
 * 없다 — 그래서 이 요약을 "그 화면들 위 대시보드"로 앞세우는 대신,
 * 지금까지 본 것 전부를 다시 한번 모아 보여주는 **마무리**로 둔다.
 * 이 저장소가 무거운 고지를 화면 마지막에 두는 관행(`ComplexDetail`의
