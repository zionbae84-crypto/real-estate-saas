 ...
 *
 * 손으로 숫자를 치지 않게 네 구간을 칩으로 낸다(스펙 §4). **복수
 * 선택**이고 기본값은 전체 선택(= 필터 없음)이다.
 *
 ...
 * 맡는다.
 *
 * `<fieldset>`/`<legend>`인 이유: 체크박스 넷이 **하나의 질문**에 대한
 * 답이라, 각 칩의 라벨만으로는 무엇을 묻는지 알 수 없다. 스크린 리더가
 * 그룹 이름을 함께 읽어야 "소형"이 무엇의 소형인지 뜻이 선다.
 *
 * 배치는 `DESIGN.md`의 Stat Block 규율을 빌린다 — **라벨 작게 위, 값
 * 크게 아래, 왼쪽 정렬.** 여기서 라벨은 범위(`~60㎡`)이고 값은 구간
 * 이름(`소형`)이다. DOM 순서는 이름 → 범위라 접근성 이름이 "소형 ~60㎡"로
 * 읽히고, 눈에 보이는 순서만 CSS가 뒤집는다.
 *
