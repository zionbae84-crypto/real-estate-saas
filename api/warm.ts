 ...
 * 인기 지역을 미리 데우는 엔드포인트.
 *
 * ⚠ **지금은 예약(크론)이 걸려 있지 않다 — 손으로 부를 때만 돈다.**
 *
 * 국토부가 속도로 막는다. 한 실행에 3~9곳에서 `HTTP 429`가 나기
 * 시작했고, 그 벽은 우리가 밀어 본 어떤 손잡이에도 꿈쩍하지 않았다:
 *
 *     동시성 8, 요청 2배 → 40곳 시도 / 9곳 성공
 *     동시성 3, 요청 2배 → 15곳 시도 / 6곳 성공
 *     동시성 3, 요청 1배 → 9곳 시도 / 3곳 성공
 *
 * 동시성을 2.7배 낮춰도 성공 개수가 그대로였다 — 순간 동시 요청 수가
 * 아니라 **시간당 총량**에 걸린다. 요청을 절반으로 줄이자 이번엔 지역당
 * 시간이 늘어 한 실행에 도는 지역이 줄었다.
 *
 * **예약을 뗀 이유는 못 데워서가 아니라 해로워서다.** 이 경로가 429를
 * 받는 동안은 같은 국토부 창구를 쓰는 **사용자 조회도 함께 막힌다.**
 * 낮 시간대(KST 15시) 실행이 실제 사용자를 막을 수 있는데, 그 대가로
 * 얻는 것은 하루 6~12곳뿐이었다.
 *
 * 되살리려면 `vercel.json`에 `crons`를 다시 넣으면 된다. 다만 그 전에
 * 위 벽을 넘을 방법이 있어야 한다 — 국토부 키의 등급을 올리거나,
 * 데우기를 훨씬 잘게 쪼개 하루 종일 나눠 돌리거나.
 *
