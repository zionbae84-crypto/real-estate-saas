...
## location.json (입지 입력 — **아직 없다**)

입지 화면(`src/components/LocationFacts.tsx`)이 소비할 파일이다. **지금은
존재하지 않는다** — 단지 좌표를 얻으려면 지번 주소 지오코딩이 필요한데
별도 API 키가 아직 없고, 학교·역 좌표 표준데이터도 아직 내려받지 않았다.
그래서 `src/data/location.ts`는 비어 있고, 화면은 모든 단지에서 "아직
위치를 몰라요"를 낸다.

이 절은 **파이프라인이 무엇을 채워야 하는지**를 못박아 둔 것이다.
...
`bubun`)다. 1,138개 단지에 결측 0건, 한 단지에 지번 하나씩 있다(예:
`서울특별시 서초구 반포동 612-2`). **주소 자체는 이 파일에 담지 않는다** —
화면에 주소를 내지 않기 때문에 번들에 실을 이유가 없다.

### subwayStations[] — 지하철역 좌표

원본: 공공데이터포털 「전국도시철도역사정보표준데이터」(역 위경도 포함).

| 필드 | 타입 | 의미 |
|---|---|---|
| id | string | 원본의 역 고유 코드. 같은 역이 노선별로 여러 행이면 각 행이 별개 항목이다 |
| name | string | 역 이름 |
| lineName | string | 노선 이름. 여러 노선이 지나는 역은 항목이 여럿이고, 엔진은 그중 가장 가까운 하나만 고른다 |
| lat / lon | number | WGS84 위경도 |

좌표가 없거나 깨진 행은 **넣지 않는다.** 엔진이 국내 범위 밖 좌표를 한 번
...
원본: 공공데이터포털 「전국초중등학교위치표준데이터」(위경도 포함).

| 필드 | 타입 | 의미 |
|---|---|---|
| id | string | 원본의 학교 고유 코드 |
| name | string | 학교 이름 |
| lat / lon | number | WGS84 위경도 |

**초등학교만 담는다.** 중·고등학교는 넣지 않는다 — 이 화면이 말하는 것은
초등학교 하나뿐이고, 학교급을 늘리면 화면이 "학군"처럼 읽히기 시작한다.
폐교·분교 여부가 원본에 있으면 폐교는 뺀다(다닐 수 없는 학교를 세면 개수가
실제보다 많아진다).
