...
> **정정(최종 리뷰, 2026-08-23):** 실제 구현은 이 정규식에서 두 단계 더
> 나아갔다. 먼저 `/(합니다|습니다)/`는 모음 어간의 "-ㅂ니다" 계열(입니다·
> 됩니다…)을 놓쳐 `/니다/`로 좁혔고(포함 관계라 커버리지는 줄지 않는다),
> 그다음 합쇼체 **의문형·명령형**("…습니까?"·"…십시오"·"…시오")까지
> 놓친다는 리뷰 지적에 따라 `/니다|니까|시오/`로 넓혔다. 한다체("…한다")는
> 이 저장소 주석 문체와 겹쳐 의도적으로 제외했고, 그 사실을
> `scripts/tone-guard.test.ts`의 정규식 주석에 알려진 공백으로 남겨
> 뒀다. 이 문서의 코드 블록은 이제 실제와 다르다 — 최신 규칙은
> `scripts/tone-guard.test.ts`를 참고할 것.
...
| 파일 | 지금 | 바꿀 것 |
|---|---|---|
| `BindingExplainer.tsx` | "상환 능력(DSR)에 걸렸습니다" | "상환 능력(DSR)에 걸렸어요" |
...
> **정정(최종 리뷰, 2026-08-23):** DSR 행은 처음엔 "소득이 한도를
> 정했어요"였다. 이는 색·모션·말투 스펙(`2026-08-23-색-모션-말투-design.md`)
> §5 "도메인 용어를 지우지 않는다"는 규칙과 정면으로 충돌했다 — DSR이라는
> 용어와 "걸렸다"는 동사가 함께 사라졌기 때문이다. 형제 행(LTV·CAP)과
> 나란한 형태로 바로잡는다. 스펙 문서도 같은 정정을 받았다.
...
> **정정(최종 리뷰, 2026-08-23):** "계산하다 문제가 생겼어요"는
> "계산하다가"의 잘림처럼 읽힌다는 리뷰 지적에 따라 "계산 중 문제가
> 생겼어요"로 바로잡았다. "…에 걸렸습니다" → "…이(가) 한도를 정했어요"도
> "걸렸다"는 동사를 지우는 같은 문제가 있어 "…에 걸렸어요"로 바로잡았다
> — 두 정정 모두 §5(위 DSR 행 정정)와 같은 이유다.
...
> **정정(최종 리뷰, 2026-08-23):** 위 예시 블록은 실제 구현(task-3-report.md
> 참고)과 세 군데 다르다.
>
> 1. `details > summary`는 **뺐다** — `src/styles.css`의 summary 셀렉터에는
>    `:hover` 등 색이 실제로 바뀌는 규칙이 없어(정적으로 `color: #666`뿐),
>    이 셀렉터에 transition을 걸어도 가드는 통과하지만 브라우저에서
>    관찰할 효과가 전혀 없다 — "가만히 있는 것은 움직이지 않는다"는 원칙에
>    어긋난다.
> 2. `.slider-price`를 대상에 **더했다** — PriceSlider가 최댓값에 닿을 때
>    같은 `<p>`에 `--max` 클래스가 붙어 브랜드색으로 실제로 바뀌는 지점이라,
>    이 블록의 대상 선정 기준("같은 DOM 노드 위에서 실제로 색이 바뀌는
>    지점")에 들어맞는다.
> 3. `transition-duration: 0.01ms !important;`는 `transition: none !important;`로
>    **바꿨다** — 앞의 형태는 숫자(0.01ms)가 남아 있어 motion-guard의
>    "전환 시간을 직접 쓰지 않는다" 검사(`transition`+숫자+단위 패턴)에
>    걸린다. `none`은 숫자 없이 같은 접근성 효과(전환 완전 제거)를 낸다.
>
> 최신 코드는 `src/styles.css`의 모션 블록(파일 끝)을 참고할 것.
